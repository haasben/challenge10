<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:73:"/www/wwwroot/api.times168.net/application/user/view/sales/edit_sales.html";i:1544437038;s:68:"/www/wwwroot/api.times168.net/application/user/view/common/head.html";i:1544436826;}*/ ?>
<!doctype html>
<html lang="en">
<head>
		...
		<div class="main" style="margin-top: 80px;">
				...
				<div class="user-wrap">
					<div class="user-info" >
						...
						<div class="info-input"><input type="text" id="username" name="username" value="<?php echo $user_data['username']; ?>" class="form-control"  placeholder="请输入用户名"></div>
					</div>
                  
                  	<div class="user-info" >
						<div class="info-title">真实姓名</div>
						<div class="info-input"><input type="text" id="realname" value="<?php echo $user_data['realname']; ?>" name="realname" class="form-control"  placeholder="请输入真实姓名"></div>
					</div>
					...
					<div class="user-info">
						...
						<div class="info-input select">
							<select name="type" class="select2 form-control" style="width: 100%;">
								<option value="1" <?php if($user_data['type'] == "1")echo "selected=\"selected\"";?> >普通玩家</option>
								<option value="2" <?php if($user_data['type'] == "2")echo "selected=\"selected\"";?> >业务员</option>
								<option value="3" <?php if($user_data['type'] == "3")echo "selected=\"selected\"";?> >个人代理</option>
								<option value="4" <?php if($user_data['type'] == "4")echo "selected=\"selected\"";?> >企业代理</option>
					...
					<div class="user-info">
						...
						<div class="info-input"><input type="text" value="<?php echo $user_data['tel']; ?>" name="tel" id="phone" class="form-control"  placeholder="请输入手机号"></div>
					</div>

					<div class="user-info" >
						<div class="info-title">返佣比</div>
						<div class="info-input"><input type="text" id="commission" value="<?php echo $user_data['commission']; ?>" name="commission" class="form-control"  placeholder="如0.5"></div>
					</div>
...
<script>
	$(function(){
		...
		$('.commission').click(function(){
			var uid = <?php echo $user_data['uid']; ?>;
			...
			$.post('<?php echo url("user/sales/edit_sales"); ?>',{uid:uid,username:username,type:type,tel:tel,commission:commission,realname:realname},function(data){
